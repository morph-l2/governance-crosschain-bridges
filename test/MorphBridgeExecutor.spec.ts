...
let l1MorphMessenger: MockMorphL1CrossDomainMessenger;
let l2MorphMessenger: MockMorphL2CrossDomainMessenger;
let bridgeExecutor: MorphBridgeExecutor;
...
const MORPH_GAS_LIMIT = 15000000;
const MORPH_VALUE = 0;

const encodeSimpleActionsSet = (
  bridgeExecutor: MorphBridgeExecutor,
  target: string,
  fn: string,
  params: any[]
) => {
...
describe('MorphBridgeExecutor', async function () {
  let snapId;
  ...
  before(async () => {
    ...
    // Mocking morph bridge messenger
    l1MorphMessenger = await new MockMorphL1CrossDomainMessenger__factory(user).deploy();
    l2MorphMessenger = await new MockMorphL2CrossDomainMessenger__factory(user).deploy();
    await l1MorphMessenger.setL2Messenger(l2MorphMessenger.address);
    await l2MorphMessenger.setL1Messenger(l1MorphMessenger.address);

    bridgeExecutor = await new MorphBridgeExecutor__factory(user).deploy(
      l2MorphMessenger.address,
  ...
  it('Check initial parameters', async () => {
    ...
    // Morph Bridge Executor parameters
    expect(await bridgeExecutor.getEthereumGovernanceExecutor()).to.be.equal(
      ethereumGovernanceExecutor.address
    );
    expect(await bridgeExecutor.L2_MORPH_MESSENGER()).to.be.equal(l2MorphMessenger.address);
  });
  ...
  context('Ethereum Governance Executor queues an actions sets', () => {
    ...
    it('Tries to queue and actions set via Morph L2 Messenger without being the Ethereum Governance Executor (revert expected)', async () => {
      ...
      await expect(
        l1MorphMessenger
          .connect(user)
          .sendMessage(bridgeExecutor.address, MORPH_VALUE, encodedData, MORPH_GAS_LIMIT, {
            gasLimit: 12000000,
          })
    ...
    it('Queue and execute an actions set to set a message in Greeter', async () => {
      ...
      const tx = await l1MorphMessenger
